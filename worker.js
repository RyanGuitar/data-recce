...
async function readFile() {
  try {
    ...
    const fileHandle = await root.getFileHandle("writer.txt");
...
async function writeFile(data) {
  const root = await navigator.storage.getDirectory();
  const draftHandle = await root.getFileHandle("writer.txt", { create: true });
  ...
  accessHandle.flush();
  accessHandle.close();
  postMessage({ msg: "write", data: "written" });
}
